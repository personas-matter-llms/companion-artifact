...
def plot_distribution(values, baselines, xlabel, filename, xlim, color, light, box_color):
    setup_style()
    model_gap = 0.5
    ...
    for pos, model in zip(positions, MODELS):
        ...
        ax.text(max(max(vals), baselines[model]) + offset, pos, label, ha="left", va="center", fontsize=8.0, color=color)
    ...
    legend = [
        Line2D([0], [0], color=light, marker="o", linewidth=0, markersize=3.4, label="Shared-Profile Config"),
    ...
    ax.legend(handles=legend, loc="lower center", bbox_to_anchor=(0.5, -0.52), ncol=4, frameon=False, fontsize=8.0)
...
def main():
    cg_values, cg_baselines = code_generation_data()
    cr_values, cr_baselines = code_review_data()

    plot_distribution(
        cg_values,
        cg_baselines,
        "Pass@1 (%)",
        "rq1_distribution_code_generation",
        (0, 50),
        "#1f5f8b",
        "#4f7f9d",
        "#d9e6ee",
    ...
    print(display_path(OUT_FIG / "rq1_distribution_code_generation.pdf"))
    print(display_path(OUT_FIG / "rq1_distribution_code_review.pdf"))

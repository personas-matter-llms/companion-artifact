...
def load_shared(path):
    ...
    for row in read_rows(path):
        ...
        grouped[(row["model"], profile)].append(float(row["bleu"]))
...
def main():
    ...
    by_model = load_shared(Path(args.data_dir) / "shared54_bleu_instances.csv")
    rows = spearman_rows(by_model)
    mean_rho = sum(row["spearman_rho"] for row in rows) / len(rows)

    print("RQ1 Spearman rank transfer: code review, shared54")
    for row in rows:
        print(

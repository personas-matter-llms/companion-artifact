...
Use `--data-path` to point the runner to the dataset (e.g. sampled LiveCodeBench JSONL file).
LiveCodeBench testcase execution is not bundled in this artifact. Please use the official LiveCodeBench evaluator if needed.
...
```bash
python scripts/run_main.py --prompt-dir <prompt_dir> --data-path <dataset_jsonl> \
...
Analysis reads `data/*.csv` and writes `results/csv/`. The R scripts additionally require the `lme4` package.

```bash
python analysis/rq1_pass_at_1.py
...
Rscript analysis/rq1_glmm_pass_at_1.R
